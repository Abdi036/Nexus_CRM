...
import { Users, UserPlus, Phone, Ticket, TrendingUp, AlertCircle } from "lucide-react"
import {
  Bar,
  BarChart,
  Line,
...
export default function DashboardPage() {
  ...
  const chartData = useMemo(() => {
    ...
    const leadsByStatus = [
      { status: "New", count: filteredLeads.filter((l) => l.status === "new").length, fill: "hsl(var(--chart-1))" },
      {
        status: "Contacted",
    ...
    const ticketsByStatus = [
      { status: "Open", count: filteredTickets.filter((t) => t.status === "open").length, fill: "hsl(var(--chart-1))" },
      {
        status: "In Progress",
    ...
    const interactionsByType = [
      { type: "Call", count: interactions.filter((i) => i.type === "call").length, fill: "hsl(var(--chart-1))" },
      { type: "Email", count: interactions.filter((i) => i.type === "email").length, fill: "hsl(var(--chart-2))" },
      { type: "Meeting", count: interactions.filter((i) => i.type === "meeting").length, fill: "hsl(var(--chart-3))" },
    ]

    // Users by role (admin only)
    const usersByRole = [
      { role: "Admin", count: users.filter((u) => u.role === "admin").length, fill: "hsl(var(--chart-1))" },
      { role: "Manager", count: users.filter((u) => u.role === "sales_manager").length, fill: "hsl(var(--chart-2))" },
      { role: "Sales Rep", count: users.filter((u) => u.role === "sales_rep").length, fill: "hsl(var(--chart-3))" },
      { role: "Support", count: users.filter((u) => u.role === "support_agent").length, fill: "hsl(var(--chart-4))" },
    ]
    ...
    const monthlyActivity = [
      ...
      { month: "Dec", leads: filteredLeads.length, customers: customers.length, interactions: interactions.length },
    ]
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: UserPlus,
      description: user.role === "sales_rep" ? "Assigned to you" : "All leads in pipeline",
    ...
    },
    {
      ...
      icon: Ticket,
      description: user.role === "support_agent" ? "Assigned to you" : "Requiring attention",
      show: ["admin", "sales_manager", "support_agent"],
    },
    {
      title: "High Priority",
      value: filteredTickets.filter((t) => t.priority === "high" && t.status !== "closed").length,
      icon: AlertCircle,
    ...
    <>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Leads, customers, and interactions over time</CardDescription>
        </CardHeader>
        <CardContent>
          <ChartContainer
            config={{
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData.monthlyActivity}>
                ...
                <Line type="monotone" dataKey="interactions" stroke="hsl(var(--chart-3))" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </ChartContainer>
        </CardContent>
